...
class InpaintGenerator(BaseNetwork):
    def __init__(self, residual_blocks=8, init_weights=True):
        ...
        for _ in range(residual_blocks):
            block = ResnetBlock(dim=256,dilation=2)
...
class EdgeGenerator(BaseNetwork):
    def __init__(self, residual_blocks=8, use_spectral_norm=True, init_weights=True):
        ...
        blocks = []
        for _ in range(residual_blocks):
            block = ResnetBlock(dim=256, dilation = 1, use_spectral_norm=use_spectral_norm)
            # wxt修改的，改爲1  2020-9-22
            # block = ResnetBlock(dim=256, dilation=2, use_spectral_norm=use_spectral_norm)
            blocks.append(block)
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dilation=1, use_spectral_norm=False):
        ...
        self.conv_block = nn.Sequential(
            nn.ReflectionPad2d(dilation), # reflection镜像填充
            spectral_norm(nn.Conv2d(in_channels=dim, out_channels=dim,
                                    kernel_size=3, padding=0, dilation=dilation, bias=not use_spectral_norm), use_spectral_norm),
            nn.InstanceNorm2d(dim, track_running_stats=False),
            nn.ReLU(True),

            nn.ReflectionPad2d(1),
            spectral_norm(nn.Conv2d(in_channels=dim, out_channels=dim,
                                    kernel_size=3, padding=0, dilation=1, bias=not use_spectral_norm), use_spectral_norm),
            nn.InstanceNorm2d(dim, track_running_stats=False),
        )
...
if __name__ == '__main__':
    ...
    modell =EdgeGenerator().to(device)
    summayy= summary(modell, (3, 128, 128))
    print(summayy )
   ...
   #################################
    ...
    writer.add_graph(net, input_to_model=input_data)
    writer.close()
    # 查看模型结构
    from tensorboard import notebook
